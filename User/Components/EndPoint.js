...
const EndPoint = ({navigation, route}) => {
    ...
    useEffect(() => {
        // const fetchData = () => {
        axios.get(`http://10.20.100.31:8080/getArrInfoByRouteAll?busRouteId=${selectedRoutedId}`) // 노선 ID
            .then(response => {
                // 가져온 데이터를 상태에 저장 response.data == bus Class
                setStation(response.data.stationNames);
                console.log('200 요청 성공' +response.data.stationNames);
            })
            .catch(error => {
    ...
    const handleItemPress = (item, index) => {
        ...
        navigation.navigate('CheckRideBus', {
            ...
            selectedRoutedId: selectedRoutedId,
